...
## The submit guard is on, with a page-level opt-out

`packages/ffp-core/src/forms.ts` replaces 5.1.5's `$(form).submit(() => false)`
with one capture-phase listener on `document`. `installFormSubmission` turns it
on and puts `submitForm` behind it, because a bubble listener on the form is
too late: Webflow binds first, treats a phone showing only `+880 ` as filled,
and swaps in `.w-form-done` before our validator can show a message. The page
can still set `data-ffp-submit-guard="off"`; the per-form listeners remain as
that fallback. linkedom does not implement the capture phase, so the property
that matters — that a handler Webflow bound on the form does not also run —
was confirmed on a published `*.webflow.io` page, not under unit tests.
