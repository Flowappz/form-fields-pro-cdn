/**
 * FORM FIELDS PRO CDN SCRIPT - v5.0.8
 */
 ...
 *
 * License / domain gating (v5.0.8):
 * - Valid license  → app loads on ANY domain (Webflow subdomain OR custom domain)
 * - No / invalid license → app loads ONLY on Webflow-hosted subdomains
 *   (e.g. https://example.webflow.io/). Custom domains stay blocked.
 */
async function initializeFormFieldsPro() {
    ...
    // Preview / staging on *.webflow.* is allowed without a license so builders can test.
    // Short-circuit: skip the license API call when already on a Webflow subdomain.
    if (isUsingWebflowDomain(url)) {
        makeTheFormInteractive();
        return;
    }

    // Custom domain: require an active license before initializing.
    const hasValidLicense = await hasValidLicenseKey(siteId);
    if (hasValidLicense) {
        makeTheFormInteractive();
        return;
    ...
    // No license + custom domain → do not initialize (fields stay non-interactive)
    console.warn(
        'Form Fields Pro: No valid license for this site. The app loads on Webflow subdomains (e.g. *.webflow.io) without a license, but a valid license is required on custom domains.',
    );
}

/**
 * Returns true when the page is served from a Webflow-hosted domain
 * (e.g. https://example.webflow.io/), not a custom domain.
 *
 * @param {string} url
 * @returns {boolean}
 */
function isUsingWebflowDomain(url) {
    // Matches hostnames like *.webflow.io / *.webflow.com used for Webflow staging/preview
