...
async function handleFormSubmit(form) {
    ...
    try {
        ...
        // Success if either Webflow OR Backend succeeded (when allowed)
        // Otherwise only Webflow matters
        if (canUseProFeatures) {
 ...
 * - Staging          → *.webflow.io subdomain
 * - Production       → custom domain
 *
 * License gating:
 * - Staging (*.webflow.io)      → always runs, license not required
 * - Production (custom domain)  → runs only when the license is active
 */
...
/**
 * Single source of truth for whether the app may run on this page.
 *
 * @param {string} siteId - Webflow site id from html[data-wf-site]
 * @returns {Promise<boolean>}
 */
async function isAppAllowedToRun(siteId) {
    // Staging publish destination — always allowed, license not required
    if (isUsingWebflowDomain()) {
        return true;
    }

    // Production publish destination (custom domain) — license required
    return hasValidLicenseKey(siteId);
}

/**
 * Returns true when the page is served from a Webflow Staging domain
 * (publish destination "Staging" — e.g. https://example.webflow.io/).
 *
 * The hostname must *end* with `.webflow.io`. A custom domain that merely
 * contains the string — e.g. `example.webflow.io.attacker.com` — is Production
 * and must still require a license.
 *
 * @param {string} [url] - defaults to the current page URL
 * @returns {boolean}
 */
...
/**
 * Cached so the license endpoint is hit once per page load instead of once per
 * bootstrap plus once per form submission.
 *
