/**
 * FORM FIELDS PRO CDN SCRIPT - v5.1.5
 * pnpm release:staging -- --version 5.1.5
...
// The licence service is per-environment like the others: a staging build that
// asks the production service gets `active: false` and silently stops posting
// submissions to the backend, with no error anywhere.
const FFP_LICENSE_URL = (() => {
    const injected = '__FFP_LICENSE_URL__'
    return injected.includes('__FFP_')
        ? 'https://license-staging.flowappz.com/api/license'
        : injected
})()
    ...
    try {
        ...
        if (hasLicense) {
            ...
            try {
                ...
                // Notification email is queued by the backend as part of the
                // submission. The browser used to make a second call to a
                // separate notifier service for it, which meant the visitor
                // waited on it and a submission could be recorded with no email
                // sent if that call was the one that failed.
            } catch (backendError) {
    ...
    licenseCheckPromise = (async () => {
        try {
            const res = await fetch(
                `${FFP_LICENSE_URL}?siteId=${siteId}&appName=form-fields-pro`,
            );
            if (!res.ok) {
