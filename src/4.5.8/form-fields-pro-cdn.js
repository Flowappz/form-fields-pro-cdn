/**
 * FORM FIELDS PRO CDN SCRIPT - v4.5.8
 */
...
async function handleFormSubmit(form) {
  ...
  try {
    ...
    const isSubmittingToNotionResponse = await fetch(
      `${BASE_URL}/api/sites/getSubmittingToNotionStatus?siteId=${siteId}`,
      { method: "GET" }
    );
    ...
    if (!isSubmittingToNotion.isSubmittingToNotion) {
      ...
      const notionResponse = await fetch(
        `${BASE_URL}/api/sites/formSubmissionToNotion`,
        {
          method: "POST",
          headers: {
...
async function hasValidLicenseKey(siteId) {
  const res = await fetch(
    `https://cache-service-production.up.railway.app/api/license?siteId=${siteId}&appName=form-fields-pro`
  );
  if (res.ok) {
    data = await res.json();
